...
def FDM_lwr(rho, D=1):
    ...
    k_x = torch.cat((torch.arange(start=0, end=k_max, step=1, device=u.device),
                     torch.arange(start=-k_max, end=0, step=1, device=u.device)), 0).reshape(1,1,nx)
    ux_h = 2j *np.pi*k_x*u_h
    rhox_h = 2j * np.pi * k_x * rho_h
    ux = torch.fft.irfft(ux_h[:, :, :k_max+1], dim=2, n=nx)
    rhox = torch.fft.irfft(rhox_h[:, :, :k_max+1], dim=2, n=nx)

    rhot = (rho[:, 2:, :] - rho[:, :-2, :]) / (2 * dt)

    f = rhot + (rhox*u + rho*ux)[:,1:-1,:]
    return f

def FDM_lwr_u0_5(rho, D=1):
    u = torch.full_like(rho, 0.5)
    batchsize = u.size(0)
    nt = u.size(1)
    nx = u.size(2)
...
def FDM_nonsep_u(rho, u, D=1):
    ...
    # V = u # 8 8 9
    # V_append_0 = torch.zeros([5, 1, 9], dtype=torch.float32)
    # V = torch.cat((V, V_append_0), 1) # 8 9 9
...
def FDM_lwr_V(V, rho, D=1):
    ...
    V_h = torch.fft.fft(V, dim=2)
    rho_h = torch.fft.fft(rho, dim=2)
    # Wavenumbers in y-direction
    k_max = nx//2
    k_x = torch.cat((torch.arange(start=0, end=k_max, step=1, device=rho.device),
                     torch.arange(start=-k_max, end=0, step=1, device=rho.device)), 0).reshape(1, 1, nx)
    Vx_h = 2j * np.pi*k_x*V_h
    rhox_h = 2j * np.pi * k_x * rho_h
    Vx = torch.fft.irfft(Vx_h[:, :, :k_max+1], dim=2, n=nx)
    rhox = torch.fft.irfft(rhox_h[:, :, :k_max+1], dim=2, n=nx)
...
def FDM_hjb_nonsep(V, rho, D=1):
    ...
    Vt = (V[:, 1:, :] - V[:, :-1, :]) / dt
    # Vt = (V[:, :, :] - torch.cat((V[:, [0], :], V[:, :-1, :]), 1)) / dt
    Vx = (V[:, :, 1:] - V[:, :, :-1]) / dx

    u = 1 - rho - Vx

    # f = Vt + (u * Vx + 0.5 * u ** 2 - u + u * rho)[:, 1:, :]
